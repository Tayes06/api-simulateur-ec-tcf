...
# Initialisation du client officiel Mistral
...
BANQUE_SUJETS = [
    ...
    {
        "tache_id": 2,
        "consigne": "Un site internet proposes un concours sur le thème 'Votre plus belle fête, racontez-la nous'. Vous participez au concours. Dans votre texte, vous racontez comment s'est passée cette fête (anniversaire, fête traditionnelle, etc.) et vous expliquez quel souvenir vous en gardez.",
        "documents_contexte": [],
    ...
    },
    {
        ...
        "documents_contexte": [
            ...
            "Document 2 : La présence d'une photo sur le CV n'est pas un critère de sélection pur mais s'avère indispensable pour certains métiers d'accueil (comme hôtesse d'accueil). Elle donne une première idée du candidat et répond au besoin de certains employeurs de visualiser à qui ils s'adresse."
        ],
        "contraintes": {"min_mots": 120, "max_mots": 180, "type_texte": "Synthèse & Prise de position"}
    },
    {
    ...
    {
        "tache_id": 3,
        "consigne": "La gratification des transports en commun. Dans la première partie, faites le résumé des documents (40-60 mots) ; dans la deuxième partie, donnez votre avis (80-120 mots).",
        "documents_contexte": [
    ...
    },
    {
        ...
        "documents_contexte": [
            ...
            "Document 2 : Choisir la campagne permet de se rapprocher de la nature et de retrouver le calme. Les relations sociales y sont plus conviviales (barbecue dans le jardin, terrasses) et le coût de l'immobilier y est nettement inférieur, offrant de plus grands espaces pour un même budget."
        ],
        "contraintes": {"min_mots": 120, "max_mots": 180, "type_texte": "Synthèse & Prise de position"}
    }
]
...
def simuler_copie_candidat(sujet, profil, niveau):
    """
    Exécute le Pipeline 2 en demandant à Mistral d'écrire une copie brute
    selon les paramètres anthropomorphiques et linguistiques du profil.
    """
    system_prompt = (
    ...
    retries = 5  # Nombre maximal de tentatives en cas de 429
    backoff_delay = 4  # Temps d'attente initial en secondes
    ...
    for attempt in range(retries):
        try:
            ...
            # On vérifie si c'est une erreur de type Rate Limit (429)
            if "429" in str(e) or "rate_limited" in str(e).lower():
                print(f"\n[!] Rate limit détecté. Attente de {backoff_delay}s avant essai {attempt + 1}/{retries}...", end="")
                time.sleep(backoff_delay)
                backoff_delay *= 2  # On augmente exponentiellement le temps d'attente
            else:
                print(f"\n[-] Erreur critique API Mistral : {e}")
                return None
                
    return None  # Si toutes les tentatives ont échoué

if __name__ == "__main__":
    print("[*] Initialisation des Pipelines 1 & 2 (Mistral - Gamme complète A1 -> C2)...")
    ...
    # Sécurité : Si vous souhaitez purger l'ancien fichier Groq corrompu/incomplet, décommentez la ligne suivante :
    # if os.path.exists(fichier_sorties): os.remove(fichier_sorties)

    total_combinaisons = len(BANQUE_SUJETS) * len(PROFILS_CANDIDATS) * len(NIVEAUX_CECRL)
    compteur = 0
    ...
    for sujet in BANQUE_SUJETS:
        ...
        for profil in PROFILS_CANDIDATS:
            for niveau in NIVEAUX_CECRL:
                compteur += 1
                print(f"[{compteur}/{total_combinaisons}] [~] Profil: {profil['profil_id']} | Niveau CECRL: {niveau}... ", end="", flush=True)
                ...
                if copie_brute:
                    ...
                    # Écrit au fil de l'eau (Append) pour préserver la donnée en cas de crash
                    with open(fichier_sorties, "a", encoding="utf-8") as f:
                        f.write(json.dumps(donnees_paire, ensure_ascii=False) + "\n")
                    
                    print("OK")
                    time.sleep(2.0)  # Fluidité des appels RPS
                else:
                    print("ÉCHEC")
